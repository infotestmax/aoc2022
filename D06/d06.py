...
def get_markers_pos(nb_cars=4):
    return [
        result[e_].append(i_ + nb_cars) for e_, x_ in enumerate(inputs)
        for i_ in range(len(x_) - nb_cars)
        if len(set(x_[i_:i_ + nb_cars])) == nb_cars
    ]

get_markers_pos()
print(f"part 1: {result}")

print(f"part 1: {[m_[0] for m_ in result]}")
...
result = [[] for _ in range(len(inputs))]
get_markers_pos(14)
print(f"part 2: {[m_[0] for m_ in result]}")
