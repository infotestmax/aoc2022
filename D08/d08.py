...
def visible_range(i_, j_):
    cell_value = data.iloc[i_][j_]
    to_check = reversed(list(
        data.iloc[0:i_][j_])), list(data.iloc[i_ + 1:][j_]), reversed(
            list(data.iloc[i_][0:j_])), list(data.iloc[i_][j_ + 1:])
    visibility = []
    for l_ in to_check:
        nested_visibility = 0
        for n_ in l_:
...
print(
    f"part 2: {max([visible_range(i_, j_) for i_ in range(1,rows_len-1) for j_ in range(1,cols_len-1)])}"
)
